...
Three of the nine ledgers are **derived** from the plan and the day folders rather than written, which
...
This project's nine ledgers split accordingly:

| | Ledgers | Source of truth |
| --- | --- | --- |
| **Hand-written** (6) | `PROGRESS` · `PACKAGES` · `DATASETS` · `MODELS` · `RUNS` · `CHANGELOG_PLAN` | observation and decision — nothing else has them |
...
def parse_plan() -> tuple[dict[int, list[str]], dict[int, str]]:
    ...
    # Slice §24.2 exactly, not all of §24. §24.1 (the phase summary) is also a table keyed
    # by a day number, and parsing it as day-map rows silently clobbers real ID assignments
    # with empty lists.
...
- `text.index("### 24.2 The map")` slices by section heading rather than by line number, so the plan
  can grow without breaking the parser. §24.2 is the last subsection of §24, so the slice ends at the
  next top-level heading, §25.
- The comment is doing real work and is worth reading twice. §24 contains **two** tables keyed by a
  day number — the phase summary and the day map — and parsing all of §24 would read phase-summary
  rows as day rows and overwrite real ID assignments with empty lists. That is a genuine bug that
  occurred while building this project, and the comment is there so it does not recur.
